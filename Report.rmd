...
kable(dmat,caption = "Cohen's d values calculated based on the ATE (columns) and pooled standard deviation (rows)",align="c")
kable(nmat,caption = "N values calculated based on the ATE (columns) and pooled standard deviation (rows)",align="c")
```
...
Through the course of the experiment, there were a number of platform limitations as well in both platforms used to conduct the survey, Mechanical Turk and Qualtrics. Many of the issues lead to an increase in respondents attriting (Figure 5) between surveys or half-way through a survey. Had these limitations been known beforehand, the experiment design could have been adjusted to accommodate. Unfortunately these limitations were discovered while conducting the experiment. 
