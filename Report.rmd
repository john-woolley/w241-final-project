...
author: "John Woolley, Clayton Monis, Derrick Hee, and Jacob Han"
...
One of the more widely discussed CAPM models up to the present time is that researched by Sharpe (1964) and Linter (1965), wherein the security market line, relating variance to expected returns, is introduced as a concept. This line is modeling the linear relationship between the expected return in excess of the risk-free rate and a quantification of risk of an asset, or in the words of Sharpe himself: “[An investor] may obtain a higher expected rate of return on his holdings only by incurring additional risk.”
...
Once aligned on this experimental design, we conducted a power calculation to determine the N that we would need to collect to find a statistically significant difference between treatment groups. We had initially aligned that we would be looking for significance at $\alpha$ = 0.05 and with a power $\beta$ = 0.80. We didn't have a good sense for the effect size (Cohen's d) that we would achieve through the experiment, so we tested different variations of effect size and pooled standard deviation to determine the Ns that would be required to identify a significant effect. We outline these calculated d and N values in Tables 1 and 2.

```{r, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)

dmat = matrix(, nrow = 5, ncol = 5)
colnames(dmat) = seq(from = 2, to = 10, by = 2)
rownames(dmat) = c('1','2','3','4','5')

nmat = matrix(, nrow = 5, ncol = 5)
colnames(nmat) = seq(from = 2, to = 10, by = 2)
rownames(nmat) = c('1','2','3','4','5')

for(row in 1:5){
  for(col in 1:5){
    dmat[row, col] = round(col / row / 2, 2)
    nmat[row, col] = round(pwr.t.test(d=col / row / 2, sig.level=0.05, power=0.8, alternative="two.sided")$n, 0)
  }
}
```
```{r}
kable(dmat,caption = "Cohen's d values calculated based on the ATE (columns) and pooled standard deviation (rows)",align="c",row.names = TRUE)
kable(nmat,caption = "N values calculated based on the ATE (columns) and pooled standard deviation (rows)",align="c",row.names=TRUE)
```

Conservatively estimating a Cohen's d of 0.5 among the collected data, we were looking to capture ~65 responses per treatment group.

# Materials

The distributions that were displayed to respondents were first generated through a Python script, included within the [Github repository for this project](https://github.com/john-woolley/w241-final-project). In particular, the `SciPy` and `SymPy` libraries to generate skew-normal distributions with given mean and variance. The quantiles of these distributions and their summary statistics (mean, median, standard deviation, and skewness) were calculated using `SciPy` and `Pandas`. Additionally, the distributions were plotted using the `matplotlib` library. The formulated visuals, as displayed to respondents, are included as Figures 2 through 5.
...
1. *Screening survey*: We set up an initial survey for use in screening respondents to assess their candidacy to participate in our research effort. This screening survey can be [previewed online through Qualtrics](https://berkeley.ca1.qualtrics.com/jfe/preview/SV_9N6bWgjwkBIqOsl?Q_CHL=preview&Q_SurveyVersionID=current). As a high-level overview, it contained the following segmentation:
    ...
    + *Familiarity with statistics*: Respondents were asked when they last took a statistics course, allowing us to permit only those who had taken a high-school level statistics course or higher, given the nature of the questions that would be asked of them in the main survey. Thereafter, the survey asked respondents a classic probability expectation question to understand if they grasp the nature of probabilistic risk and the potential outcomes of an investment.
...
Randomization inference was used to generate p-values for each of the treatment conditions in comparison to control. Of High Variance, Left-Skew, and Right-Skew (Figure 8), both Left-Skew (0.047) and Right-Skew (0.014) returned significant p-values at the 95% confidence level. Additionally, pairwise t-tests (Table 4) were also conducted on the samples. We report Bonferroni adjusted p-values. When uncorrected, both Left-Skew and Right-Skew t-tests returned significant p-values at the 95% confidence level. However when corrected, neither were significant. 

```{r}
knitr::opts_chunk$set(echo = FALSE)
h_c<-t.test(highvar_group[, dif], control_group[, dif], alternative = "two.sided")$p.value
l_c<-t.test(leftskew_group[, dif], control_group[, dif], alternative = "two.sided")$p.value
r_c<-t.test(rightskew_group[, dif], control_group[, dif], alternative = "two.sided")$p.value

p_table<-data.frame(
 "Uncorrected" = c(round(h_c,4),round(l_c,4),round(r_c,4)),
 "Bonferroni-Corrected" = c(round(h_c*3,4),round(l_c*3,4),round(r_c*3,4))
 )
row.names(pidt)<-c('Control','High Variance','Left-Skew','Right-Skew')
kable(p_table,caption = "P-values Generated by T-Test on Collected Data",align="c")
```
...
One limitation of our study was too small to reach the desired power.  Assuming an average $d$ of 0.5, we would have required about 63 respondents per group, but got about half of that amount.  The return rate of qualified respondents on our pilot survey was 80% but we only achieved a 50% response rate with the main survey.  Also, we ran out of budget on the overall sample because we had to assume that every respondent would return to prevent any of them from being locked out from the MTurk batch given MTurk's prepaid system.

Through the course of the experiment, there were a number of platform limitations as well in both platforms used to conduct the survey, Mechanical Turk and Qualtrics. Many of the issues lead to an increase in respondents attriting (Figure 6) between surveys or half-way through a survey. Had these limitations been known beforehand, the experiment design could have been adjusted to accommodate. Unfortunately these limitations were discovered while conducting the experiment. 
...
## Citations
-  Black, F., Jensen M. C., & Scholes, M. (1972). "The Capital Asset Pricing Model: Some Empirical Tests." *Studies in the Theory of Capital Markets*. Praeger.
- Bossaerts, P., & Plott, C. (2002). The CAPM in thin experimental financial markets. *Journal of Economic Dynamics and Control*, 26(7–8), 1093–1112. https://doi.org/10.1016/s0165-1889(01)00046-x
- Difallah, D., Filatova, E., & Ipeirotis, P. (2018, February 2). Demographics and Dynamics of Mechanical Turk Workers. *Proceedings of the Eleventh ACM International Conference on Web Search and Data Mining*. WSDM 2018: The Eleventh ACM International Conference on Web Search and Data Mining. https://doi.org/10.1145/3159652.3159661 
...
- Kozik, T. & Larson, A. (2001). The n-moment insurance CAPM. *Proceedings of the Casualty Actuarial Society*. 88. 
